...
export const proData = (state = defaultState, action) => {
    ...
    switch (action.type) {
        ...
        case pro.EDITPRODUCTION:
            //避免引用类型数据，使用immutable进行数据转换 
            imuDataList = Immutable.List(state.dataList); //all json
            imuItem = Immutable.Map(state.dataList[action.index]); //json[index]
            imuItem = imuItem.set('selectNum', action.selectNum); //json[index].selectNum(+1/-1)
            imuDataList = imuDataList.set(action.index, imuItem); //edit all json
            // redux必须返回一个新的state
            return { ...state, ...{ dataList: imuDataList.toJS() } };
        // 清空数据
