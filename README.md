...
一个简单的React上手项目，功能主要以体验配置react-redux,react-router，store，上手开发jsx为主
...
```
 git clone git@github.com:tedQB/Redux-test.git
...
## React的 Diff算法
react的diff算法用在什么地方呢？当组件更新的时候，react会创建一个新的虚拟dom树并且会和之前储存的dom树进行比较，这个比较多过程就用到了diff算法，所以组件初始化的时候是用不到的。
